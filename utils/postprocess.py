...
def postprocess_magma_ocean(
    ...
    meltfrac_solid = np.zeros(n_steps)
    ...
    for i in range(n_steps):
        ...
        if is_magma_ocean:
            _, meltfrac_solid[i] = get_meltfracb(gp, temp_mantle, Rp, Rc, Mmantle, params)
        ...
        visc = nu * rho_mantle
        solid_shear_modulus = calc_shear_modulus(
            meltfrac[i],
            params
        )
    ...
    return {
        ...
        't_crust': t_crust, 't_degas': t_degas, 'melt_fraction': meltfrac_solid
    }
