...
function [dTr_dt] = moODEb2(t,Tr,Rp,Rc,Mmantle,Teq,rho,g,Ts,Ps,OLR,ASR,t_flux,L_GJ,Xi,FeOt,Temp_K,P_Pa,tsat,a,Mp,LStar)
...
Watm = Tr(3); % mass of water in the atmosphere
if Watm < 0
    Watm = 0.0;
end

FH2O = Tr(2)/Mmantle; %mass fraction of water in the mantle
MO_tot = Tr(4); % mass of free O in atmosphere
if MO_tot < 0
    MO_tot = 0;
end
Tsurf = Tr(5);
...
%find the atmospheric pressure (Pa)
    if Tr(5) > 647
        ...
        Patm = 10^(6.079-2261.10/Tr(5))*1e5;
...
%find degassing flux of water
if FH2O > 1e-9
    [~,rmor,~,~] = degas2(Tm,Db,q_mantle,FH2O,Rp,g,Tsurf);
else
    rmor = 0.0;
end
...
% model. 
    ...
    dTr_dt(2) = -rmor * 4 * pi * Rp^2 * uc * Xd * 3.15569e7;

%     if Tr(3) > 1e3
%         dTr_dt(3) = (rmor * 4*pi*Rp^2*uc*Xd - 4 * pi * Rp^2 * phi_H*muH2O/2/muH) * 3.15569e7;%rate of loss of H2O from atmosphere
%     else
%         dTr_dt(3) = (rmor * 4*pi*Rp^2*uc*Xd) * 3.15569e7;%rate of loss of H2O from atmosphere
%     end
% 
%     if Tr(3) > 1e3 && Tr(4) > 0
%         dTr_dt(4) = (4 * pi * Rp^2 * phi_H * muO/2/muH - 4 * pi * Rp^2 *...
%             phi_O) * 3.15569e7; %gain of O from loss of atmospheric H
% %         dTr_dt(4) = (4 * pi * Rp^2 * phi_H * muO/2/muH) * 3.15569e7; %gain of O from loss of atmospheric H
%     elseif Tr(3)< 1e3 && Tr(4) > 0
%         dTr_dt(4) = (- 4 * pi * Rp^2 *...
%             phi_O) * 3.15569e7; %gain of O from loss of atmospheric H
%     else
%         dTr_dt(4) = 0.0;
%     end
    %mass of water in the atmosphere
    if Tr(3) > 1e3 && Tr(5) > 647
        dTr_dt(3) = (rmor * 4*pi*Rp^2*uc*Xd - 4 * pi * Rp^2 * phi_H*muH2O/2/muH) * 3.15569e7;%rate of loss of H2O from atmosphere
    else
        dTr_dt(3) = (rmor * 4*pi*Rp^2*uc*Xd) * 3.15569e7;%rate of loss of H2O from atmosphere
    end

    %mass of O in the atmosphere
    if Tr(3) > 1e3 && Tr(4) > 0 && Tr(5) > 647 
        dTr_dt(4) = (4 * pi * Rp^2 * phi_H * muO/2/muH - 4 * pi * Rp^2 *...
            phi_O) * 3.15569e7; %gain of O from loss of atmospheric H
    elseif Tr(3)< 1e3 && Tr(4) > 0 && Tr(5) > 647
        dTr_dt(4) = (- 4 * pi * Rp^2 *...
            phi_O) * 3.15569e7; %gain of O from loss of atmospheric H
    else
        dTr_dt(4) = 0.0;
    ...
    dTr_dt(5) = 4*pi * Rp^2 * 3.15569e7* ((-flux + q_mantle)/(cpH2O * Patm *...
